...
<sql:query dataSource="${conexao}" var="qr">
   SELECT COUNT(usu_codigo) as 'qtd' FROM Usuario WHERE usu_nome like '%${where}%';
</sql:query>    
...
<c:choose>
    <c:when test="${ordem == 'cod'}">
        <c:set var="order_by" value="usu_codigo" />
    </c:when>
    <c:when test="${ordem == 'nom'}">
        <c:set var="order_by" value="usu_nome" />
    </c:when>        
    <c:when test="${ordem == 'usu'}">
        <c:set var="order_by" value="usu_usuario" />
    </c:when>        
    <c:otherwise>
...
<sql:query dataSource="${conexao}" var="r">
    SELECT usu_codigo, usu_nome, usu_usuario FROM Usuario WHERE usu_nome like '%${where}%' ORDER BY ${order_by} LIMIT ${inicio}, ${exiba};
</sql:query>
...
<div class="col-sm-9 col-sm-offset-3 col-md-12 col-md-offset-2 main">
	...
	<div class="col-md-3 mt-3"><h2>Usuário</h2></div>
            ...
            <div class="col-md-3 mt-3">
                    <a href="../Incluir/index.jsp" class="btn btn-primary pull-right h2">Inserir Usuário</a>
            </div>
        </div>			
                        ...
                        <table class='table table-striped'>
                            ...
                            <tbody>
                                <c:forEach var="i" items="${r.rows}">
                                    <tr>
                                        <td><c:out value="${i.usu_codigo}"/></td> 
                                        <td><c:out value="${i.usu_nome}"/></td>
                                        <td><c:out value="${i.usu_usuario}"/></td>
                                        <td class='actions'>
                                            <a href="../Alterar/index.jsp?id=<c:out value="${i.usu_codigo}"/>" class="btn btn-xs btn-warning">Editar</a>
                                            <a href="javascript:confirmacao('Deseja apagar o usuário?',
                                                     '../Excluir/index.jsp?id=<c:out value="${i.usu_codigo}"/>')"
