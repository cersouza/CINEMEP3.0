...
<c:choose>
    ...
    <c:otherwise>
        ...
        <sql:query dataSource="${conexao}" var="qr">
            SELECT COUNT(gen_codigo) as qtd FROM genero WHERE gen_Descricao like '%${where}%';
        </sql:query>    
...
<c:choose>
    <c:when test="${ordem == 'cod'}">
        <c:set var="order_by" value="gen_codigo" />
    </c:when>
    <c:when test="${ordem == 'des'}">
        <c:set var="order_by" value="gen_descricao" />
    </c:when>        
    <c:otherwise>
        <c:set var="ordem" value="des" />
        <c:set var="order_by" value="gen_descricao" />
...
<sql:query dataSource="${conexao}" var="r">
    SELECT gen_codigo, gen_descricao FROM genero WHERE gen_descricao like '%${where}%' ORDER BY ${order_by} LIMIT ${inicio}, ${exiba};
</sql:query>
...
<div class="col-sm-9 col-sm-offset-3 col-md-12 col-md-offset-2 main">
	...
	<div class="col-md-3 mt-3"><h2>Genero</h2></div>
            ...
            <div class="col-md-3 mt-3">
                    <a href="../Incluir/index.jsp" class="btn btn-primary pull-right h2">Inserir Genero</a>
            </div>
        </div>			
                        ...
                        <table class='table table-striped'>
                            <thead>
                                    <tr>
                                            ...
                                            <th width='60%'>
                                                <a href='index.jsp?ordem=des'>Descrição</a>
                                            </th>
                            ...
                            <tbody>
                                <c:forEach var="i" items="${r.rows}">
                                    <tr>
                                        <tr>
                                        <td><c:out value="${i.gen_codigo}"/></td> 
                                        <td><c:out value="${i.gen_descricao}"/></td>
                                        <td class='actions'>
                                            <a href="../Alterar/index.jsp?id=<c:out value="${i.gen_codigo}"/>" class="btn btn-xs btn-warning">Editar</a>
                                            <a href="javascript:confirmacao('Deseja apagar o genero?',
                                                     '../Excluir/index.jsp?id=<c:out value="${i.gen_codigo}"/>')"
